...
class TestCaptureTimestamp:
    ...
    def test_encode_decode_large(self):
        ...
        # 2024-01-01 00:00:00 UTC のマイクロ秒
...
class TestVideoConfig:
    ...
    def test_encode_decode_avc(self):
        ...
        # AVCDecoderConfigurationRecord の例
        config = VideoConfig(
            codec_config=b"\x01\x42\xE0\x1E\xFF\xE1\x00\x10"
        )
        encoded = config.encode()
        decoded = VideoConfig.decode(encoded)
        assert decoded.codec_config == b"\x01\x42\xE0\x1E\xFF\xE1\x00\x10"

    def test_encode_decode_hevc(self):
        """HEVC (H.265) の Video Config"""
        config = VideoConfig(
            codec_config=b"\x01\x01\x60\x00\x00\x00\x00\x00"
        )
        encoded = config.encode()
        decoded = VideoConfig.decode(encoded)
...
class TestVideoFrameMarking:
    """Video Frame Marking のテスト"""

    def test_encode_decode_keyframe(self):
        """キーフレーム"""
    ...
    def test_encode_decode_delta_frame(self):
        """デルタフレーム"""
        marking = VideoFrameMarking(
            independent=False,
            discardable=True,
    ...
    def test_encode_decode_with_layers(self):
        """時間・空間レイヤー付き"""
        marking = VideoFrameMarking(
            independent=True,
            discardable=False,
...
class TestAudioLevel:
    ...
    def test_encode_decode_silent(self):
        """無音"""
    ...
    def test_encode_decode_voice(self):
        """音声あり"""
    ...
    def test_encode_decode_max_level(self):
        """最大レベル"""
...
class TestLocExtensionsIntegration:
    """LOC Extensions の統合テスト"""

    def test_create_video_extensions(self):
    ...
    def test_create_video_extensions_with_config(self):
    ...
    def test_create_audio_extensions(self):
    ...
    def test_parse_video_extensions(self):
    ...
    def test_parse_audio_extensions(self):
    ...
    def test_roundtrip_through_object_extensions(self):
